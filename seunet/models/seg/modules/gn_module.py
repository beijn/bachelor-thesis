...
from timm.models.layers import norm, trunc_normal_
...
class GNConvModule(nn.Module):
    def __init__(self,
        ...
        if type == 'gnconv':
            self.dwconv = get_dwconv(sum(self.dims), kernel_size,
                                     True)  # nn.Conv2d(sum(self.dims), sum(self.dims), kernel_size=7, padding=3, groups=sum(self.dims))
        # elif type == 'gngf':
        #     self.dwconv = GlobalLocalConv2Norm(sum(self.dims), h=h, w=w,
        #                                        kernel_size=gf_kernel_size)  # nn.Conv2d(sum(self.dims), sum(self.dims), kernel_size=7, padding=3, groups=sum(self.dims))
        else:
            raise NotImplementedError()
        ...
        if self.with_norm:
            ...
            self.norm_name, norm = 'BN2d', nn.BatchNorm2d(norm_channels)
